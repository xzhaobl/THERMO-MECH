...
C USER-DEFINED MATRIX     
     ...
     1 STRESS2(NTENS)
      ...
      DIMENSION DDSTRAN(NTENS),ESTRESS(NTENS), ETH_PLAS(NTENS)
...
999   CONTINUE    
     ...
     1 FGS1,FF1,FG1,NDI,NSHR,NTENS)
      ...
      CALL GETTH_PLAS(FG1,FF1,BETA_T,FR,TH_PLAS1,DDTEMP,NDI,NSHR,NTENS)
     ...
     !!& -TH_PLAS1(I)
      ...
      FVOID2=EXP(-DEV+alpha*DDTEMP/(1+FVOID1))*(1+FVOID1)-1 !对数应变&true strain
...
C 第二次应力计算 
     ...
     1 FGS2,FF2,FG2,NDI,NSHR,NTENS)
      ...
      CALL GETTH_PLAS(FG2,FF2,BETA_T,FR,TH_PLAS2,DDTEMP,NDI,NSHR,NTENS)
      
      Do I=1,NTENS
          ESTRESS(I)=0.5*(DSTRESS2(I)-DSTRESS1(I))
     & -0.5*(TH_PLAS2(I)-TH_PLAS1(I)) !!!!!!!
          ETH_PLAS(I)=0.5*(TH_PLAS2(I)-TH_PLAS1(I))
     ...
     & -0.5*TH_PLAS1(I)-0.5*TH_PLAS2(I)
      ...
      Do I=1,NTENS
          ...
          FEI_T1=FEI_T1+ETH_PLAS(I)*ETH_PLAS(I)
          FEI_T2=FEI_T2+0.5*(TH_PLAS1(I)+TH_PLAS2(I))*
     & 0.5*(TH_PLAS1(I)+TH_PLAS2(I))
      End Do
      !FERR=SQRT(FEIE/FEIS)
      !If(DDTEMP.GT.SSTOL)Then
      !    FERR2=SQRT(FEI_T1/FEI_T2)
      !else
      !    FERR2=1E-8
      !end if
      !If(FERR.LE.FERR2)FERR=FERR2
...
C 更新数据       
     ...
     1 FF,FG,NDI,NSHR,NTENS)
      CALL GETTH_PLAS(FG,FF,BETA_T,FR,TH_PLAS,DTEMP,NDI,NSHR,NTENS)
      STATEV(1)=FVOID2
      STATEV(2)=FPC
      STATEV(3)=TEMP+DTEMP
      STATEV(4)=FGMOD
...
!unload-reload      
      ...
      SUBROUTINE GETTH_PLAS(FG,FF,BETA_T,FR,TH_PLAS,DTEMP,NDI,NSHR,NTENS)
      INCLUDE 'ABA_PARAM.INC'
      DIMENSION FG(NTENS),TH_PLAS(NTENS)
      Do I=1,NTENS
          TH_PLAS(I)=FG(I)/FF*BETA_T/LOG(FR)*DTEMP
      End Do
