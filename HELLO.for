...
999   CONTINUE    
     ...
     1FGS1,FF1,FG1,NDI,NSHR,NTENS)
      ...
      FPC2=FPC1+FPC1*((1+FVOID1)/(FLama-FKapa)*DEVP1-BETA_T*DDTEMP)
...
C 第二次应力计算 
     ...
     1FGS2,FF2,FG2,NDI,NSHR,NTENS)
      ...
      Do I=1,NTENS
          TEMP_ST(I)=FG2(I)/FF2*BETA_T/LOG(FR)*DDTEMP
          STRESS(I)=STRESS(I)+0.5*DSTRESS1(I)+0.5*DSTRESS2(I)-TEMP_ST(I)
      End Do
      DEVP=0.5*(DEVP1+DEVP2) !硬化参数更新
      FPC=FPC1+FPC1*((1+FVOID1)/(FLama-FKapa)*DEVP-BETA_T*DDTEMP)
