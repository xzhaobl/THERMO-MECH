...
C 第一次应力计算     
      ...
      FGMOD1=FKMOD1*3.0*(1-2.0*Fv)/2.0/(1+Fv) !G
     ...
     !!1FATA*FATA)/2.0/FATA
      FPFP1=FM*FM*2.0*FP1-FM*FM*FPC1
      FPFQ1=2.0*FQ1
      FATA=FQ1/FP1 !应力比
      FPB1=FM*FM*FPC1/(FATA**2+FM**2)
      FSDB1=SQRT(1+FATA**2)*FPB1 ! distance of image point to the original point
      FKP1=FM**4*(1+FVOID1)/(Flama-Fkapa)*FPC1*FP1*(2*FP1-FPC1)
...
999   CONTINUE    
      ...
      CALL GETDEP(FP1,FQ1,STRESS,FPFP1,FPFQ1,DE1,FKP1,DEP1,DDSTRAN,
     1FGS1,FF1,FG1,NDI,NSHR,NTENS)
            
      DEVP1=FPFP1*FGS1 !plastic volumetric STRAIN increment & hardening parameter
      
      FPC2=FPC1+FPC1*((1+FVOID1)/(FLama-FKapa)*DEVP1)
...
C 第二次应力计算 
      ...
      FGMOD2=FKMOD2*3.0*(1-2.0*Fv)/2.0/(1+Fv) !G      
      
      CALL GETDE(FKMOD2,FGMOD2,DE2,NDI,NSHR,NTENS)
      
     !! FPFP2=FN*((FQ2/FM/FP2)**(FN-1))*(-FQ2/FM)/FP2/FP2+1/FP2/LOG(FR)
     !! FPFQ2=FN*((FQ2/FM/FP2)**(FN-1))/FM/FP2
     !! FATA2=FQ2/FP2
     !! 
     ...
     !! FDS2=(FM*FM*(FSD2/FSDB2)**2-FATA*FATA)/2.0/FATA
      FPFP2=FM*FM*2.0*FP2-FM*FM*FPC2
      FPFQ2=2.0*FQ2
      FATA=FQ2/FP2 !应力比
      FPB2=FM*FM*FPC2/(FATA**2+FM**2)
      FSDB2=SQRT(1+FATA**2)*FPB2 ! distance of image point to the original point
      FKP2=FM**4*(1+FVOID2)/(Flama-Fkapa)*FPC2*FP2*(2*FP2-FPC2)     
  
      CALL GETDEP(FP2,FQ2,STRESS1,FPFP2,FPFQ2,DE2,FKP2,DEP2,DDSTRAN,
     1FGS2,FF2,FG2,NDI,NSHR,NTENS)
      ...
      FPC=FPC1+FPC1*((1+FVOID1)/(FLama-FKapa)*DEVP)
...
C 更新数据       
      ...
      FGMOD=FKMOD*3.0*(1-2.0*Fv)/2.0/(1+Fv)
      CALL GETDE(FKMOD,FGMOD,DE,NDI,NSHR,NTENS)
      If(FQ.LE.1e-5)FQ=1e-5
      
      FPFP=FM*FM*2.0*FP-FM*FM*FPC
      FPFQ=2.0*FQ
      FATA=FQ/FP
      
      FSD=SQRT(FP**2+FQ**2)     
      FPB=FM*FM*FPC/(FATA**2+FM**2)
      FSDB=SQRT(1+FATA**2)*FPB
      FKP=FM**4*(1+FVOID)/(Flama-Fkapa)*FPC*FP*(2*FP-FPC)

     
      CALL GETDEP(FP,FQ,STRESS,FPFP,FPFQ,DE,FKP,DDSDDE,DSTRAN,FGS
...
C恢复ABAQUS原本的符号
      ...
      SUBROUTINE GETDEP(FP,FQ,FSTRESS,FPFP,FPFQ,FDE,FKP,DEP,DER,FGS,
...
C
     ...
     !! SUBROUTINE CORRECTION(FSTRESS,FPFP,FPFQ,FBOUND,FDS,FKP,FDE,
     !!1CORRECT_ST,CORRECT_EVP,NDI,NSHR,NTENS)
     !! INCLUDE 'ABA_PARAM.INC'
     !! DIMENSION FSTRESS(NTENS),FDE(NTENS,NTENS)
     !! DIMENSION CORRECT_ST(NTENS)
     !! DIMENSION CA(NTENS),CB(NTENS),CC(NTENS),CD(NTENS),CE(NTENS),
     !!1CG(NTENS),CH(NTENS,NTENS),CI(NTENS)
     !! 
     !! Do I=1,NDI
     !! CA(I)=1.0/3.0
     !! End Do
     ...
     !! Do I=1,NDI
     !! CB(I)=3.0/2.0/SINV2*(FSTRESS(I)-SINV1)
     !! End Do
     !! Do I=1,NSHR
     !! CB(NDI+I)=3.0/2.0/SINV2*FSTRESS(NDI+I)
     !! End Do            
     !! Do I=1,NTENS
     !!     CC(I)=FDS*CA(I)+CB(I)
     !!     CD(I)=FPFP*CA(I)+FPFQ*CB(I)
     !! End Do        
     !! Do I=1,NTENS
     !!     CE(I)=0.0
     !!     Do J=1,NTENS
     !!         CE(I)=CE(I)+CD(J)*FDE(I,J)
     !!     End Do
     !! End Do
     !!
     !! CF=0.0
     ...
     !!     CORRECT_ST(I)=0
     !!     Do J=1,NTENS
     !!     CORRECT_ST(I)=CORRECT_ST(I)-FDE(I,J)*CC(J)*FGS_C
     !!     End Do
     !! End Do
     !! End